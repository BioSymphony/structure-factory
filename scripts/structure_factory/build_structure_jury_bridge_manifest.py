...
RUN_ID = "bsf-demo-structure-jury-dual-dossier"
LINEAR_ISSUE_URL = ""
...
PY
python3 scripts/structure_factory/structure_jury_dossier.py --out runpod-execution --json | tee runpod-execution/artifacts/campaign-summary.stdout.json
"""
    return {
        "schema_version": 1,
        ...
        "workload": {
            ...
            "stage_contract": {
                ...
                "exact_commands": [
                    "python3 scripts/structure_factory/structure_jury_dossier.py --out runpod-execution --json",
                ],
                "expected_outputs": [
        ...
        "launch_authorization": {
            ...
            "scope": "CPU-only dual public PDB/EMDB structure jury, no licensed tools, no raw movies, target under two hours",
        },
        ...
        "startup": {
            ...
            "commands": [
                "set -euo pipefail",
                "cd /workspace/repo",
                "mkdir -p runpod-execution/logs && { nohup python3 -m http.server \"${RUNPOD_HTTP_ARTIFACT_SERVER_PORT:-8000}\" --bind 0.0.0.0 >> runpod-execution/logs/live-http.log 2>&1 </dev/null & echo $! > runpod-execution/live-http.pid; disown \"$(cat runpod-execution/live-http.pid)\" 2>/dev/null || true; }",
                workload_command,
            ],
        },
        "monitoring": {
        ...
        "worker_coordination": {
            ...
            "resource_name_prefix": "bsf-demo-structure-jury",
        },
        "closeout": {
...
def main() -> int:
    ...
    files = {
        "scripts/structure_factory/structure_jury_dossier.py": (ROOT / "scripts" / "structure_factory" / "structure_jury_dossier.py").read_text(),
        "scripts/structure_factory/t2r14_open_dossier.py": (ROOT / "scripts" / "structure_factory" / "t2r14_open_dossier.py").read_text(),
        "scripts/structure_factory/poltheta_map_model_dossier.py": (ROOT / "scripts" / "structure_factory" / "poltheta_map_model_dossier.py").read_text(),
    }
